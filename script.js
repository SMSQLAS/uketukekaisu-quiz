const questions = [
  {
   ...
   choices: ["そのまま", "80/100"],
   answer: "80/100",
    explanation: "<strong>異なる医療機関から発行された処方箋であれば、受付回数は２回以上となりますが、受け付けが１回目以外の処方箋の調剤基本料は所定点数を100分の80として算定します。</strong>"
  },
  {
  ...
  {
    text: "A診療所B科（診察は午前のみ）を受診、処方箋を午前中に持参した。いったん出勤し、夕方A診療所C科（診察は午後のみ）を受診し、処方箋を持参した。<br>両科の受診についてはB科からC科へ受診勧告があったわけではなく、全く別々の疾病による自発的受診で関連はない。<br>この場合の受付回数は何回？",
   choices: ["1回", "2回"],
   answer: "1回",
  ...
  {
    text: "ここから修正！！同一医療機関・同一診療科で、午前と午後に別の医師から処方箋が交付された。患者への聞き取りで、午後分は体調変化による追加処方だと確認できても、別受付として扱うには医療機関へ必ず電話確認しなければならない。○か×か。",
    answer: false,
